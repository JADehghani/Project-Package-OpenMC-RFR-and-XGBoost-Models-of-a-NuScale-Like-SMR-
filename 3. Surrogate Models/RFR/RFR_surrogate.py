#!/usr/bin/env python3
"""
Random Forest Regressor Training and Hyperparameter Optimization for Nuclear Reactor Surrogate Modeling

This script develops and optimizes a Random Forest Regressor (RFR) surrogate model for predicting the neutron 
...
Methodology:
    ...
    - Implements GridSearchCV across a comprehensive hyperparameter space including:
        ...
        * Maximum features per split (sqrt, log2, all features)
        * Maximum tree depth (unlimited, 10, 20)
        * Minimum samples required for splitting (2, 5, 10)
...
Additional Files:
    ...
    - rfr_k_eff_surrogate.pkl (trained model joblib output)
    - Chart_Feature_Importance_RFR.png (feature importance visualization output, using matplotlib)
...
import joblib # For Saving the RFR algorithm
import time # For precise timing (as a measure of computational demand)
from sklearn.ensemble import RandomForestRegressor # import the RFR from sci-kit learn.
...
# Function alculates error in pcm (parts per hundred thousand, good for nuclear reactor applications). 
# Arguments:
...
def nuclear_scoring_metric(y_true, y_pred):
...
scorer = make_scorer(nuclear_scoring_metric, greater_is_better=False)
...
# Configures Random Forest Regressor with a set random state for reproducibility (Breiman, 2001), and sets up 
# 5-fold cross-validation to prevent bias in the distribution of data (Bisong, 2019).
# ------------------------------------------------------------ #

# Initialize base RFR model (random state = 42)
...
param_grid = {
    ...
    'max_features': ['sqrt', 'log2', 1.0],  # Features randomly selected at each split
    'max_depth': [None, 10, 20],  # Maximum tree depth / complexity (None = unlimited)
...
grid_search = GridSearchCV(
    estimator=rfr, # asign base model as estimator
...
best_rfr = grid_search.best_estimator_  # Extract optimized model
...
joblib.dump(best_rfr, 'rfr_k_eff_surrogate.pkl')
print("\nModel saved as pickle file: 'rfr_k_eff_surrogate.pkl'")
...
importances = best_rfr.feature_importances_  # Extract Gini importance scores
...
# Create horizontal bar chart with navy blue color scheme
...
plt.title('A: RFR Feature Importance', fontsize=14, pad=15, fontname='sans-serif')
...
plt.tight_layout()
plt.savefig('Feature_Importance_RFR.png', dpi=300, bbox_inches='tight')
